...
get_probability = function(df){
  ...
  bart = readRDS("bart.rds")
  preproc = readRDS("preproc.rds")
  pre = c("ageyr", "intNoNLN", "necrosisp", "tstage", "sitea", "intNoPLN")
  df[pre] = predict(preproc, df[pre])
  pred_prob = predict(bart, df)
  
  quant = ifelse(pred_prob < 0.758, "High risk",
                 ifelse(pred_prob < 0.884, "Intermediate risk",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$do, {
    df = data.frame(input$intNoNLN, input$intNoPLN, input$tstage, input$msin, 
                    input$necrosisp, input$ageyr, input$sitea)
    
    names(df) =  c( "intNoNLN", "intNoPLN", "tstage", "msin", "necrosisp", "ageyr", "sitea")
